...
const actions = {
  GET_GROUP({ commit }) {
    Vue.http.get('http://localhost:8081/ContactsBe/getGroup').then(response => {
  ...
  ADD_GROUP({ commit }, params) {
    ...
    Vue.http.post('http://localhost:8081/ContactsBe/addGroup', params).then(response => {
  ...
  DEL_GROUP({ commit }, group, index) {
    Vue.http.post('http://localhost:8081/ContactsBe/delGroup', group).then(response => {
