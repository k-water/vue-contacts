...
const actions = {
  ...
  GET_PERSON({ commit }) {
    Vue.http.get('http://localhost:8080/ContactsBe/getPerson').then((res) => {
  ...
  FUZZY_QUERY({ commit }, fkey) {
    Vue.http.post('http://localhost:8080/ContactsBe/SearchPerson', { key: fkey }).then(res => {
      commit(types.GET_PERSON, { contacts: JSON.parse(res.body) })
    }, err => {
  ...
  async ADD_PERSON({ commit }, list) {
    ...
    await Vue.http.post('http://localhost:8080/ContactsBe/addPerson', list).then(response => {
  ...
  DEL_PERSON({ commit }, person) {
    Vue.http.post('http://localhost:8080/ContactsBe/delPerson', person).then(response => {
  ...
  UPDATE_PERSON({ commit }, person) {
    Vue.http.post('http://localhost:8080/ContactsBe/updatePerson', person).then(response => {
